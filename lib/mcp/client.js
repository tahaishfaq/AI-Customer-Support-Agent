...
/** Full text of an MCP tool result (no 4 KB cut) — for list detection; capped at 3 MB. */
export function mcpResultText(result) {
  if (result == null) return "";
  if (typeof result === "string") return result.slice(0, 3_000_000);
  ...
  try {
    return JSON.stringify(result).slice(0, 3_000_000);
  } catch {
    return "";
  }
}
