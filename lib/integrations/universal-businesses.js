...
export function buildUniversalSlotTemplates(business) {
  ...
  return [
    ...
    },
    {
      slot: "GUEST_LOOKUP",
      name: guestName,
      description: `Guest lookup for ${business.name} (demo order status — replace with a redacted tracking API; never return another customer's PII).`,
      method: "GET",
    ...
    },
    {
      slot: "ACCOUNT_READ",
      name: accountName,
      description: `Signed-in read for ${business.name} (requires setUser + your ACL; denied without verified identity). Demo URL for studio smoke.`,
      method: "GET",
      urlTemplate: "http://127.0.0.1:3000/api/demo/orders/{{orderId}}",
      headersJson: { Accept: "application/json" },
      inputSchemaJson: { orderId: "string" },
