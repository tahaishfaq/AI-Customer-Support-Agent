...
import { companyPackForBusiness, serializeCompanyPackForInstall, validateCompanyPack } from "@/lib/integrations/company-pack";
import {
  classifyUrlConnectionStatus,
  packConnectionMeta,
} from "@/lib/integrations/pack-status";
...
async function createActionsFromSlotTemplates({
  ...
  for (const template of templates) {
    ...
    const serialized = serializeActionForOwner(action);
    created.push({
      ...serialized,
      connectionStatus: classifyUrlConnectionStatus(urlTemplate),
    });
  }

  const statusMeta = packConnectionMeta({
    packId,
    urls: [
      ...created.map((a) => a?.urlTemplate),
      ...templates.map((t) => resolvePackUrlTemplate(t?.urlTemplate)),
    ],
  });
...
export async function createActionPack({
  ...
  if (businessId) {
    ...
    const companyPack = serializeCompanyPackForInstall(
      companyPackForBusiness(business)
    );
    const packCheck = validateCompanyPack(companyPack);
    if (!packCheck.ok) throw httpError(400, "Company pack is invalid", packCheck);
