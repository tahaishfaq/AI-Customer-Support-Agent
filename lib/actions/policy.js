...
import {
  ...
  isAideDemoActionUrl,
} from "./access-class.js";
import { detectCrossUserRequest } from "./response-sanitize.js";
import { assertResourceSubjectBinding } from "./authz-binding.js";

/** Name tokens that suggest personal data: such tools keep Confirm on the embed (F14). */
const PERSONAL_DATA_NAME_RE =
  /(^|_)(my|me|mine|account|accounts|profile|user|users|customer|customers|member|members|order|orders|invoice|invoices|billing|payment|payments|subscription|subscriptions|address|addresses|booking|bookings|reservation|reservations|ticket|tickets|appointment|appointments|cart|wallet|balance|refund|refunds|shipment|shipments|tracking)(_|$)/i;

export const IDENTITY_PROOF_REQUIRED = "IDENTITY_PROOF_REQUIRED";
export const ACCOUNT_TOOL_MISCONFIGURED = "ACCOUNT_TOOL_MISCONFIGURED";
...
export function evaluateActionPolicy({
  ...
  // Phase 0 — embed: block account-looking PUBLIC_READ tools that run on owner keys.
  if (
    publicAccess &&
    !action?._mcp &&
    !action?._builtin &&
    accessClass === "PUBLIC_READ" &&
    (identityMode === "NONE" || identityMode === "OWNER_KEY") &&
    ...
    return {
      ...
      message:
        "This account-data tool is misconfigured for embed (PUBLIC_READ without visitor identity). Set access class to ACCOUNT_READ.",
      needsConfirmation: false,
      needsIdentity: true,
      identityMode,
    };
  ...
  // F11-U: live HTTP tools on the public embed require Confirm (studio can auto-run).
  // Owner-connected MCP READ (e.g. GitHub get_me) uses the agent's credential, not the
  // visitor's account — Confirm stays for WRITE/DESTRUCTIVE MCP.
  const isOwnerMcpRead =
    Boolean(action?._mcp) &&
    risk === "READ" &&
    !Boolean(action?.requiresConfirmation);
  // F14 (2026-09-24): public catalog reads (plans, help articles, hours) run on the embed
  // without Confirm. Only when the owner explicitly stored PUBLIC_READ (never inferred), the
  // call is a GET with no visitor identity, and the name does not suggest personal data.
  const isPublicCatalogRead =
    ...
    (identityMode === "NONE" || identityMode === "OWNER_KEY") &&
    !ACCOUNT_RESOURCE_TOOL_NAME_RE.test(String(action?.name || "")) &&
    !PERSONAL_DATA_NAME_RE.test(String(action?.name || ""));
  const needsWriteConfirm =
    Boolean(action?.requiresConfirmation) ||
    risk === "WRITE" ||
    risk === "DESTRUCTIVE" ||
    (Boolean(publicAccess) && !isOwnerMcpRead && !isPublicCatalogRead);
