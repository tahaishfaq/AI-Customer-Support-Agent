...
export function detectCrossUserRequest(
  ...
  if (text.trim()) {
    const crossPhrases = [
      ...
      /\bon behalf of\b/,
      /\bsomeone('?s)? (order|account|invoice|ticket|data)\b/,
      /\b(other|another) customer('?s)?\b/,
      /\btheir (order|account|invoice|ticket|email|phone)\b/,
      /\b(access|view|read)\b.+\b(as|for)\b.+\b(admin|other user)\b/,
    ];
