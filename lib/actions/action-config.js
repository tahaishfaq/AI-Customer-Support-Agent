...
/**
 * Owner or workspace ADMIN/OWNER seat may edit/run actions for this agent.
 * Platform admin stays inspect-only — not an actor here.
 * Prefer getAgentForUser(..., { mutate: true }) at the service boundary.
 */
export function canManageAgentActions({ userId, agent, canManage = null }) {
  if (!userId || !agent) return false;
  if (canManage === true) return true;
  if (canManage === false) return false;
  return Boolean(agent.userId && agent.userId === userId);
}
...
export function serializeActionForOwner(action) {
  ...
  return {
    ...
    currentDraftRevisionId: action.currentDraftRevisionId ?? null,
    publishedRevisionId: action.publishedRevisionId ?? null,
...
export const ACTION_TEMPLATES = Object.freeze([
  ...
  },
  {
    ...
    description:
      "Look up a Shopify order by id via the Shopify Admin connector when a *.myshopify.com connection + access-token credential are attached. Requires Aide-signed visitor identity on embed.",
    method: "GET",
  ...
  },
  {
    ...
    description:
      "Order or status lookup for the signed-in visitor (demo order API). Requires Aide-signed identity on embed; point URL at your store API in production.",
    method: "GET",
