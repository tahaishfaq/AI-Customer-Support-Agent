...
export function toolsPromptAddon(actionNames = []) {
  ...
  const lines = [
    "## Live actions (tools)",
    `You may call these server tools when the user needs live data or account changes the knowledge base cannot provide: ${list}.`,
    "Prefer knowledge for FAQs and policies. Use a tool when the user asks to look up or change live account/data (orders, preferences, tickets, status).",
    "If update_preference (or similar write tools) is listed and the user asks to change a preference, call it — do not refuse or escalate instead.",
  ...
  if (hasHandoff) {
    lines.push(
      "Call request_handoff only after knowledge and other listed tools cannot finish the request. Prefer the matching data/write tool over handoff."
    );
  }
  return lines.join(" ");
}
