...
export function safeToolErrorMessage(result) {
  ...
  if (code === "IDEMPOTENCY_IN_FLIGHT") {
    return "This write is already in progress. Wait a moment — do not invent success.";
  }
  if (code === "DAILY_LIMIT") {
  ...
  if (code === "CONFIRMATION_REQUIRED") {
    return "This action needs explicit user confirmation. A Confirm button is shown in chat — tell the user what will happen and wait for them to confirm. Do not call request_handoff and do not invent success.";
  }
  ...
  if (code === "RESULT_BINDING_MISMATCH") {
    return "The live response did not match the verified customer. Do not reveal or use it.";
  }
  if (
    ...
    return "This action is not allowed in this conversation. Answer from knowledge or ask a clarifying question.";
  }
  ...
  if (code === "MCP_TOOL_ERROR") {
    return "The connected MCP tool returned an error. Retry a different listed tool that matches the ask (e.g. search_repositories for repo lists). Do not invent repository or account inventory.";
  }
...
export function formatToolResultForModel(result, opts = {}) {
  ...
  if (result?.ok) {
    ...
    return JSON.stringify({
      ...
      replyHint: guest
        ...
        : "Answer ONLY what the user asked. If they asked for status, give status (and name) only — no extra fields. If no result was found, say that clearly without inventing details.",
    });
  }
  ...
  const includeDetail =
    (http >= 400 && http < 500) ||
    String(result?.errorCode || "").toUpperCase() === "MCP_TOOL_ERROR";
  ...
  return JSON.stringify({
    ...
    ...(String(result?.errorCode || "").toUpperCase() === "MCP_TOOL_ERROR"
      ? {
          replyHint:
            "MCP tool failed. Call a different listed tool that matches the user ask. Do not invent GitHub inventory from memory or Agent knowledge.",
        }
      : {}),
  });
}
