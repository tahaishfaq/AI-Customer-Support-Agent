...
/**
 * MCP tools that create a new unique resource (e.g. GitHub create_repository).
 * Replay of a prior OK would lie on a second user confirm with the same name —
 * GitHub should be called again so "already exists" can surface.
 */
export function mcpWriteSkipsIdempotencyReplay(action) {
  const remote = String(action?._mcp?.remoteName || "").toLowerCase();
  const name = String(action?.name || "").toLowerCase();
  return (
    /(^|_)create_repository$/.test(remote) ||
    /create_repositor/.test(remote) ||
    /create_repositor/.test(name)
  );
}
