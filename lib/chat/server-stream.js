...
export function createChatServerStream(execute, { signal: requestSignal, timeoutMs = 60_000, maxBufferedBytes = 1_048_576, onError } = {}) {
  ...
  const emit = event => {
    ...
    try { controller.enqueue(bytes); } catch (error) { console.warn('[chat-stream] enqueue failed:', error.message, { eventType: event.type, bytesLength: bytes.byteLength }); abort(); return; }
