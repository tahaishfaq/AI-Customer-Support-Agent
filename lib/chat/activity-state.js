...
const NEXT = {
  ...
  // Same-mode tool chain (e.g. MCP get_me then search_repositories): reopen one row.
  completed: new Set(['selected', 'validating', 'running', 'needs_confirmation', 'needs_identity']),
  failed: new Set(['selected', 'validating', 'running']),
};
...
export function normalizeActivityEvent(value) {
  ...
  return {
    kind: 'agent_activity',
    ...(value.schemaVersion == null ? {} : { schemaVersion: Number(value.schemaVersion) || 1 }),
    ...(validId(value.eventId) ? { eventId: value.eventId } : {}),
    ...(validId(value.conversationId) ? { conversationId: value.conversationId } : {}),
    ...(validId(value.messageId) ? { messageId: value.messageId } : {}),
    turnId: value.turnId, activityId: value.activityId,
    sequence: value.sequence,
    ...(Number.isSafeInteger(value.seq) ? { seq: value.seq } : {}),
    mode: value.mode, phase: value.phase,
...
export function reduceActivityEvent(state, rawEvent) {
  ...
  if (event.sequence <= previous.sequence || event.mode !== previous.mode) return state;
...
export function activityLabel(event) {
  ...
  if (event.phase === 'selected') return {
    web_search: 'Starting web search', knowledge: 'Opening knowledge',
    http: 'Selecting connected service', mcp: 'Selecting connected tool',
    hybrid: 'Comparing sources', handoff: 'Requesting human support',
  }[event.mode] || 'Starting';
  return {
    preparation: 'Thinking…', knowledge: 'Checking knowledge',
    http: 'Calling connected service', mcp: 'Running connected tool',
