...
export function createActivityStream(emit, initialContext = {}) {
  ...
  const stream = event => {
    ...
    const seq = ++sequence;
    const safe = normalizeActivityEvent({
      ...data,
      schemaVersion: 1,
      eventId: `event-${turnId}-${seq}`,
      turnId,
      activityId: ids.get(key),
