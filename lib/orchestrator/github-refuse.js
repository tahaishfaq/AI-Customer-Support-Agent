...
export function githubWriteRefuseMessage(toolSteps = [], opts = {}) {
  ...
  if (detail && isNameConflictDetail(detail)) {
    ...
    const options =
      alts.length > 0
        ? ` Options: (1) ${alts[0]}  (2) ${alts[1]}. Reply with 1, 2, or type a different name you want — I will ask for Confirm only after you choose.`
        : " Reply with a different repository name you want — I will ask for Confirm only after you choose.";
    if (embed) {
      return `That repository name${named ? ` (${taken})` : ""} is already taken on GitHub.${options} I will not invent a success.`;
    }
    return `A repository${named} already exists on this GitHub account. Please create it under a different name.${options} I will not invent a success URL.`;
  }
  if (!detail) {
