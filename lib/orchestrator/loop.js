...
export async function runOrchestratorLoop({
  ...
  // #region agent log
  {
    const beforeGithub = (Array.isArray(actions) ? actions : [])
      .filter((a) => a?._mcp && /github/i.test(String(a?.name || "")))
      .map((a) => ({ name: a.name, risk: a.riskLevel }));
    const afterGithub = routedActions
      .filter((a) => a?._mcp && /github/i.test(String(a?.name || "")))
      .map((a) => ({ name: a.name, risk: a.riskLevel }));
    const strippedWrite = beforeGithub.filter(
      (b) =>
        ["WRITE", "DESTRUCTIVE"].includes(String(b.risk || "").toUpperCase()) &&
        !afterGithub.some((a) => a.name === b.name)
    );
    fetch("http://127.0.0.1:7921/ingest/84e093c9-c14f-4ada-99a7-450980e35e0c", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        runId: "pre-fix",
        hypothesisId: "H1",
        location: "loop.js:routedActions",
        message: "orchestrator source-route tool filter",
        data: {
          utterance: String(lastUserMessage || "").slice(0, 160),
          route: sourceDecision?.route,
          wantsGithub: Boolean(sourceDecision?.signals?.wantsGithub),
          beforeGithubCount: beforeGithub.length,
          afterGithubCount: afterGithub.length,
          afterGithubNames: afterGithub.map((a) => a.name).slice(0, 20),
          strippedWriteNames: strippedWrite.map((a) => a.name).slice(0, 15),
          createBefore: beforeGithub.some((a) => /create_repositor/i.test(a.name)),
          createAfter: afterGithub.some((a) => /create_repositor/i.test(a.name)),
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    ...
    for (const call of orderedCalls) {
      ...
      fetch("http://127.0.0.1:7921/ingest/84e093c9-c14f-4ada-99a7-450980e35e0c", {
        ...
        body: JSON.stringify({
          ...
          runId: "pre-fix",
          hypothesisId: "H2-H5",
          location: "loop.js:toolCall",
          message: "model requested tool",
          data: {
            name,
            isHandoff: /request_handoff/i.test(name),
            isCreateRepo: /create_repositor/i.test(name),
            isGithubMcp: /mcp_.*github/i.test(name),
            inByName: byName.has(name),
            wantsGithub,
            route: sourceDecision?.route,
