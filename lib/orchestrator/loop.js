...
import { githubInventoryRefuseMessage, githubWriteRefuseMessage } from "@/lib/orchestrator/github-refuse";
import { listTitleFor } from "@/lib/mcp/list-result";
...
/** Repo-list tools only — releases/collaborators must not satisfy a "list repos" ask. */
const GITHUB_REPO_LIST_NAME = /search_repositor|list_repositor/;
...
/** True when a repo-list tool succeeded (not releases/collaborators/get_me alone). */
function hasSuccessfulGithubRepoList(toolSteps = []) {
  return toolSteps.some((step) => {
    if (!isSuccessfulStep(step)) return false;
    const name = String(step.name || "").toLowerCase();
    if (!name.includes("mcp_") || !name.includes("github")) return false;
    return GITHUB_REPO_LIST_NAME.test(name);
  });
}

function userAskedForRepoList(utterance) {
  const t = String(utterance || "").toLowerCase();
  // Create/update/delete are WRITE asks — do not require search_repositories success.
  if (userAskedForGithubWrite(utterance)) return false;
  return (
    /\b(repositor(?:y|ies)|repos?)\b/.test(t) &&
    /\b(list|show|get|fetch|search|first|top|my)\b/.test(t)
  );
}

/** Create/update GitHub repo (or named create_repository tool) — not a list ask. */
function userAskedForGithubWrite(utterance) {
  const t = String(utterance || "").toLowerCase();
  if (/create_repositor/.test(t)) return true;
  return (
    /\b(create|creating|new|make|add|update|delete|remove)\b/.test(t) &&
    /\b(repositor(?:y|ies)|repos?)\b/.test(t)
  );
}

function hasSuccessfulGithubWrite(toolSteps = []) {
  return toolSteps.some((step) => {
    if (!isSuccessfulStep(step)) return false;
    const name = String(step.name || "").toLowerCase();
    if (!name.includes("mcp_") || !name.includes("github")) return false;
    return /create_repositor|push_files|create_or_update_file|update_file|delete_file|fork_repositor/.test(
      name
    );
  });
}

function githubRefuseForTurn(utterance, toolSteps, opts) {
  if (userAskedForGithubWrite(utterance)) {
    return githubWriteRefuseMessage(toolSteps, opts);
  }
  return githubInventoryRefuseMessage(toolSteps, opts);
}
...
/**
 * Server-authored text (refusal, fallback) or clearing a draft: one `replace` event.
 * Never fake-chunk finished text — only real model tokens stream as deltas.
 * @param {string} text
 * @param {(ev: { type: string, data: object }) => void} [onEvent]
 */
function emitReplaceText(text, onEvent) {
  if (typeof onEvent !== "function") return;
  onEvent({ type: "delta", data: { text: String(text || ""), replace: true } });
}
...
async function completeAssistantText({
  ...
  if (streaming && typeof onEvent === "function") {
    try {
      ...
      if (content) emitReplaceText(content, onEvent);
      return content;
    }
  }
...
export async function runOrchestratorLoop({
  ...
  if (wantsGithub && !hasMcpOffered) {
    ...
    if (wantStream) emitReplaceText(refuse, onEvent);
    return {
  ...
  if (hostedWebRoute) {
    // Hosted search events can arrive with the first answer tokens; show "Searching the web"
    // as soon as the turn is routed so the visitor isn't left on "Thinking…" for seconds.
    if (wantStream) {
      emitToolActivity(onEvent, { name: "web_search", status: "running", step: 0, route: sourceDecision.route });
    }
  ...
  while (stepsUsed < maxSteps) {
    ...
    if (!turn.toolCalls?.length) {
      ...
      if (
        ...
        stepsUsed += 1;
        const repoListNudge = userAskedForRepoList(lastUserMessage);
        loopMessages.push({
          role: "assistant",
          content,
        });
        loopMessages.push({
          role: "user",
          content: repoListNudge
            ? "[System] You must call search_repositories or list_repositories (for example search_repositories with user:LOGIN) before answering. Do not call list_releases or list_repository_collaborators for a repository list. Do not invent repo names or URLs."
        ...
        if (wantStream) emitReplaceText("", onEvent);
        continue;
      }
      // Repo-list ask + wrong tools (releases/collaborators) → force a real repo-list call.
      if (
        wantsGithub &&
        userAskedForRepoList(lastUserMessage) &&
        attemptedMcpTool(toolSteps) &&
        !hasSuccessfulGithubRepoList(toolSteps) &&
        stepsUsed < maxSteps &&
        [...byName.keys()].some((n) =>
          /search_repositor|list_repositor/i.test(String(n || ""))
        )
      ) {
        stepsUsed += 1;
        loopMessages.push({
        ...
        loopMessages.push({
          role: "user",
          content:
            "[System] Previous tools did not return a repository list. Call search_repositories (user:LOGIN from get_me if available) or list_repositories now. Do not invent repos or URLs.",
        });
        if (wantStream) emitReplaceText("", onEvent);
        continue;
      }
      ...
      if (
        ...
        !(userAskedForRepoList(lastUserMessage)
          ? hasSuccessfulGithubRepoList(toolSteps)
          : userAskedForGithubWrite(lastUserMessage)
            ? hasSuccessfulGithubWrite(toolSteps)
            : hasSuccessfulGithubInventory(toolSteps))
      ) {
        const refuse = githubRefuseForTurn(lastUserMessage, toolSteps, {
          publicAccess,
        });
        // Replace any draft tokens instead of appending the refusal to them.
        if (wantStream) emitReplaceText(refuse, onEvent);
        return {
          assistantText: refuse,
          toolSteps,
    ...
    // Text streamed before the model chose tools is a draft, not the answer.
    if (wantStream && turn.content) emitReplaceText("", onEvent);
    ...
    for (const call of orderedCalls) {
      ...
      const step = await invokeOneTool({
        ...
        maxSteps,
        suppressedPublicReadNames: [...suppressedNames],
        // Large list results stream to the chat in chunks (display-only data).
        onListProgress: wantStream
          ? (chunk) =>
              onEvent({
                type: "list",
                data: {
                  listId: callId,
                  title: listTitleFor(byName.get(name)?._mcp?.remoteName || name),
                  items: chunk.items,
                  total: Number.isFinite(chunk.total) ? chunk.total : null,
      ...
      if (step?.args && typeof step.args === "object") {
        try {
          step._argsRaw = JSON.stringify(step.args);
        } catch {
          /* keep original */
        }
      } else if (step?.pendingConfirmation?.args) {
        try {
          step._argsRaw = JSON.stringify(step.pendingConfirmation.args);
        } catch {
          /* keep original */
        }
      }
  ...
  const githubInventoryOk = userAskedForRepoList(lastUserMessage)
    ? hasSuccessfulGithubRepoList(toolSteps)
    : userAskedForGithubWrite(lastUserMessage)
      ? hasSuccessfulGithubWrite(toolSteps)
      : hasSuccessfulGithubInventory(toolSteps);
    ...
    stopReason === "needs_user"
      ...
      : stopReason === "escalate"
        ...
        : githubFailedInventory
          ? userAskedForRepoList(lastUserMessage)
            ? "Connected GitHub MCP did not return a repository list (search_repositories / list_repositories). Tell the user you could not list repositories. Do not invent repo names or URLs."
            : userAskedForGithubWrite(lastUserMessage)
              ? "Connected GitHub MCP did not complete the repository write (create/update). If the tool said the name already exists / is taken: tell the user that name is taken, offer 1–2 alternate name options, and ask them to reply with an option or their own name — do NOT call create_repository again or show Confirm until they choose. Do not invent success or repo URLs."
  ...
  // A completed handoff already saved its server-written ack; reuse it instead of a
  // second model call (~1 s) that would only rephrase it.
  const handoffAck =
    stopReason === "escalate"
      ? toolSteps.find((s) => s?.handoff?.triggered && s.handoff.ackMessage?.content)?.handoff
          .ackMessage.content || ""
      : "";
  let content = githubFailedInventory
    ? githubRefuseForTurn(lastUserMessage, toolSteps, { publicAccess })
    : handoffAck;
  ...
  if (!content) {
    try {
      ...
      degraded = true;
      if (content && wantStream) emitReplaceText(content, onEvent);
    }
  } else if (wantStream) {
    emitReplaceText(content, onEvent);
  }
  ...
  if (!content) {
    ...
    degraded = true;
    if (wantStream) emitReplaceText(content, onEvent);
  }
