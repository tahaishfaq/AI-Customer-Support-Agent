...
const GITHUB_INVENTORY_NAME =
  /search_repositor|list_repositor|search_code|search_pull|search_user|search_issue|search_commit|get_file_contents|list_commits/;
...
function emitToolActivity(onEvent, { name, status, step, route, action, result, itemId }) {
  ...
  onEvent({
    ...
    data: {
      kind: "agent_activity",
      activityId: itemId ? `search-${itemId}` : `step-${step || 0}-${mode}`,
      mode,
      ...resolved,
      name: safeToolName,
...
export async function runOrchestratorLoop({
  ...
  if (wantsGithub && !hasMcpOffered) {
    if (wantStream) emitTextAsDeltas(GITHUB_INVENTORY_REFUSE, onEvent);
    return {
      assistantText: GITHUB_INVENTORY_REFUSE,
      toolSteps: [],
      clientActions: [],
      degraded: true,
  ...
  const hostedWebRoute =
    ...
    // Do not skip the tool loop when GitHub/MCP inventory is in play.
    !wantsGithub &&
    !hasMcpOffered;
  ...
  let content = githubFailedInventory
    ? GITHUB_INVENTORY_REFUSE
    : await completeAssistantText({
        system: `${system}\n\n${limitNote}`,
        messages: loopMessages,
        signal,
        streaming: wantStream,
  ...
  if (githubFailedInventory && wantStream) {
    emitTextAsDeltas(content, onEvent);
  }
  ...
  return {
    ...
    toolSteps,
    clientActions: clientActionsFromSteps(toolSteps),
    degraded: Boolean(githubFailedInventory),
    latencyMs: Date.now() - started,
