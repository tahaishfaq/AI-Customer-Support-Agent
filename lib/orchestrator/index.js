...
import { routeSource } from "@/lib/services/ai/source-policy";
 ...
 * @property {{ customerSubject?: string|null, endUserAccessToken?: string|null, customerClaims?: { email?: string|null, phone?: string|null }|null, identityStrategy?: "hs256_jwt"|"host_session"|null }} [identity]
 ...
 * @property {string[]} [suppressedPublicReadNames] — server-derived redundant public reads
...
export async function runTurn(ctx) {
  ...
  const result = await runOrchestratorLoop({
    ...
    suppressedPublicReadNames: actionsEnabled
      ? ctx.suppressedPublicReadNames ?? []
      : [],
    signal: ctx.signal,
    agentId: ctx.agentId,
    ...
    customerClaims: identity.customerClaims ?? null,
    identityStrategy: identity.identityStrategy ?? null,
    publicAccess: Boolean(flags.publicAccess),
    lastUserMessage: ctx.userMessage ?? null,
    sourceDecision: ctx.sourceDecision ?? null,
  ...
  safeLog("info", "orchestrator.turn", {
    ...
    sourceRoute:
      ctx.sourceDecision?.route ||
      routeSource(ctx.userMessage ?? null).route,
