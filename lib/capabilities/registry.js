...
export async function listCapabilitiesForAgent(agentId) {
  ...
  const agent = await prisma.agent.findUnique({
    where: { id: agentId },
    select: { actionsEnabled: true, workspaceId: true, webSearchEnabled: true },
  });
  if (!agent) {
    return { descriptors: [], actions: [], workspaceId: null };
  }
  ...
  const materializedHttpActions = await Promise.all(
    httpActions.map((action) => materializePublishedAction(action))
  );
  const actions = [...builtins, ...materializedHttpActions, ...mcpTools];
  return {
