...
export async function invokeBuiltinCapability(ctx) {
  ...
  if (builtinId === "web_search") {
    return invokeHostedWebSearchGuard({
      action,
      auditBase,
      auditToolRun,
      logToolStep,
...
async function invokeHostedWebSearchGuard({
  action,
  auditBase,
  auditToolRun,
  logToolStep,
  ...
  const durationMs = Date.now() - started;
  const errorCode = "HOSTED_WEB_SEARCH_PROVIDER_DISPATCH_REQUIRED";
  ...
  return withCapabilityResult({
    ...
    resultForModel: JSON.stringify({
      ok: false,
      errorCode,
      message: "Hosted web search must be dispatched by the Responses API route.",
    }),
  });
}
