...
export async function getAdminQueueCounts() {
  const data = await apiFetch("/api/admin/queues");
  return {
    enabled: data.queues?.enabled === true,
    queues: Array.isArray(data.queues?.queues)
      ? data.queues.queues
...
export async function getAdminBillingPlans() {
  const data = await apiFetch("/api/admin/billing/plans");
  return data.plans || [];
}

export async function updateAdminBillingPlan(id, patch) {
  const data = await apiFetch(`/api/admin/billing/plans/${id}`, {
    method: "PATCH",
    body: JSON.stringify(patch),
  });
  return data.plan;
...
export async function listAdminCustomPlanRequests({ status = "NEW" } = {}) {
  const params = new URLSearchParams();
  if (status) params.set("status", status);
  const data = await apiFetch(`/api/admin/billing/requests?${params}`);
  return data.requests || [];
}

export async function updateAdminCustomPlanRequest(id, patch) {
  const data = await apiFetch(`/api/admin/billing/requests/${id}`, {
    method: "PATCH",
    body: JSON.stringify(patch),
  });
  return data.request;
