...
export async function listKnowledge(agentId) {
  ...
  return {
    ...
    latestCrawl: data.latestCrawl || null,
  };
}
...
/** Owner crawl / re-crawl — public HTTPS pages only. Accepts one URL or urls[]. */
export async function retrySiteCrawl(agentId, { homepageUrl, urls } = {}) {
  const body = {};
  const list = Array.isArray(urls)
    ? urls.map((u) => String(u || "").trim()).filter(Boolean)
    : [];
  const url = String(homepageUrl || "").trim();
  if (list.length) body.urls = list;
  else if (url) body.homepageUrl = url;
  return apiFetch(`/api/agents/${agentId}/crawl/retry`, {
    method: "POST",
