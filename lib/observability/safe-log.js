...
function sanitizeMeta(meta = {}) {
  ...
  // spans: timing milestones only — keep short names with finite numbers.
  if (out.spans) {
    const spans = {};
    for (const [name, value] of Object.entries(out.spans).slice(0, 20)) {
      if (/^[A-Za-z][\w]{0,31}$/.test(name) && Number.isFinite(value)) spans[name] = value;
    }
    out.spans = spans;
  }
  return out;
