...
async function notifyCustomPlanRequest(request) {
  ...
  const data = {
    ...
    estimatedSeats: request.estimatedSeats,
  ...
  for (const admin of admins) {
    try {
      const result = await sendEmail({
        template: EMAIL_TEMPLATES.CUSTOM_PLAN_REQUEST_ADMIN,
        to: admin,
        userId: request.userId,
        data,
        idempotencyKey: `custom_plan_admin:${request.id}:${admin}`,
        tags: ["billing", "custom_plan"],
      });
      if (result.ok) sent = true;
    } catch (error) {
      console.error("[billing] custom plan admin email failed", error.message);
    }
  }
  ...
  if (request.contactEmail) {
    try {
      await sendEmail({
        template: EMAIL_TEMPLATES.CUSTOM_PLAN_REQUEST_ACK,
        to: request.contactEmail,
        userId: request.userId,
        data: { contactName: request.contactName },
        idempotencyKey: `custom_plan_ack:${request.id}`,
        tags: ["billing", "custom_plan_ack"],
      });
    } catch (error) {
      console.error("[billing] custom plan ack email failed", error.message);
    }
  }
