...
import { extractEventMeta } from "@/lib/billing/webhook-meta";
export { extractEventMeta } from "@/lib/billing/webhook-meta";
...
async function updateSubscriptionWithRealtime({ subscriptionId, data, eventType, payload = {} }) {
  return prisma.$transaction(async (tx) => {
    ...
    await enqueueRealtimeEvent(tx, {
      eventType: eventType || REALTIME_EVENT_TYPES.BILLING_SUBSCRIPTION_UPDATED,
...
async function applySafepayEvent(meta, { ip }) {
  ...
  if (eventType === "subscription:created") {
    await updateSubscriptionWithRealtime({
      subscriptionId: subscription.id,
      data: baseUpdate,
      payload: { action: "token_recorded", providerEventType: eventType },
    });
    return {
      ignored: false,
  ...
  if (
    ...
    if (subscription.status === "ACTIVE") {
      await updateSubscriptionWithRealtime({
        ...
        payload: { action: "payment_failed", providerEventType: eventType },
      });
  ...
  if (eventType === "subscription:unpaid") {
    await updateSubscriptionWithRealtime({
      subscriptionId: subscription.id,
      data: { ...baseUpdate, status: "PAST_DUE" },
      payload: { action: "past_due", providerEventType: eventType },
    });
    await notifySubscriptionPastDue({
      userId: subscription.userId,
      subscriptionId: subscription.id,
  ...
  if (eventType === "subscription:ended") {
    ...
    await updateSubscriptionWithRealtime({
      ...
      payload: { action: "ended", providerEventType: eventType },
    });
