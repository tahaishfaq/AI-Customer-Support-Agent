 ...
 * @returns {Promise<object|null>} the quota when it was checked; null when this message cannot hit it.
 */
export async function assertConversationQuota(
  ...
  if (!isNewConversation) {
    ...
    // Only the visitor's 2nd message makes a conversation billable, so only that message can hit
    // the limit. Count this conversation first (indexed, cheap) and skip the owner-wide count otherwise.
    const { periodStart, periodEnd } = getCurrentBillingMonthWindow();
    ...
    if (userMsgsInPeriod !== 1) return null;
  }
