...
import { isWhiteLabelPlan } from "@/lib/customization/white-label";
...
/** Paid plans (and admins) may white-label the widget: logo, launcher image, footer, branding. */
export async function resolveWhiteLabelAccess(userId, role = null) {
  if (!userId) return false;
  const effectiveRole = role || (await getUserRole(userId));
  if (effectiveRole === "ADMIN") return true;
  const billing = await getBillingSnapshot(userId, effectiveRole);
  return isWhiteLabelPlan({
    role: effectiveRole,
    unlocked: billing.unlocked,
    planType: billing.entitlements?.planType || null,
  });
}
