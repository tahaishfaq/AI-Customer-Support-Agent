...
function countUnsafeAccountTools(actions) {
  let n = 0;
  for (const action of actions) {
    if (!action.enabled) continue;
    const check = assertAccountToolPublishSafe(action);
    if (!check.ok) {
      n += 1;
      continue;
    }
...
async function loadSignals(agent) {
  ...
  return {
    ...
    unsafeAccountTools: countUnsafeAccountTools(actions),
...
function assemble(agent, signals, integrations, { stale }) {
  const evaluated = evaluateEmbedReadiness({
    ...
    unsafeAccountTools: signals.unsafeAccountTools || 0,
