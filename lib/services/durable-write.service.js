...
export async function listUnknownWrites({ limit = 50 } = {}) {
  const take = Math.min(Math.max(Number(limit) || 50, 1), 100);
  return prisma.durableWriteOperation.findMany({
    where: { status: "OUTCOME_UNKNOWN", reconciliationStatus: "REQUIRED" },
