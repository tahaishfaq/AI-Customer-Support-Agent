...
import {
  loadExactKpis,
  loadPlatformGrowthSql,
  loadSqlCharts,
  loadTrendPointsSql,
} from "@/lib/services/analytics-sql";
...
function buildRangeSpec(rangeInfo) {
  const trendStart =
  ...
  return {
    ...
    buckets: rangeInfo.buckets,
    grid,
    pointKey: (date) => pointKey(date, rangeInfo.bucket),
  };
}
...
function buildAgentRowsFromStats(agents, agentStats, responseByAgent, totalConversations) {
  return agents
    .map((agent) => {
      const row = agentStats.get(agent.id) || {
        conversations: 0,
        messages: 0,
        positives: 0,
        negatives: 0,
...
async function loadBundleFromAgents(agents, since) {
  ...
  if (agentIds.length === 0) {
    return { agents, assistantTimes: [], exact: null };
  }

  const [exact, assistantTimes] = await Promise.all([
    loadExactKpis(agentIds, since),
    loadAssistantTimes(agentIds, since),
  ]);

  return { agents, assistantTimes, exact };
}
...
async function buildDashboardAnalytics({
  ...
  const agentIds = chartAgentIds ?? agents.map((agent) => agent.id);
  const rangeSpec = buildRangeSpec(rangeInfo);
  const sqlCharts = await loadSqlCharts(agentIds, since, rangeSpec, agents, {
    weekdays: WEEKDAYS,
    hourLabel,
  });
  ...
  const totalConversations = exact?.conversationCount ?? 0;
  const overview = buildOverview([], responseTimes, exact);
  const topics = buildTopicsFromExact(exact, []);
  const sentiment = buildSentimentFromExact(exact, []);
  const agentRows = buildAgentRowsFromStats(
    agents,
    sqlCharts.agentStats,
    responseByAgent,
    totalConversations
  );
  ...
  const insights = buildInsights({
    ...
    topics,
    sentiment,
    trends: sqlCharts.trends,
    agents: agentRows,
    heatmap: sqlCharts.heatmap,
    scoped,
  });
  ...
  return {
    dashboard: {
      ...
      activeAgents: agentRows.filter((agent) => agent.conversations > 0).length,
      overview,
      topics,
      sentiment,
      trends: sqlCharts.trends,
      sentimentTrend: sqlCharts.sentimentTrend,
      workloadTrend: sqlCharts.workloadTrend,
      heatmap: sqlCharts.heatmap,
      responseBuckets,
      agents: agentRows,
      insights,
    ...
    rangeSpec,
    chatTrendPoints: sqlCharts.trends.points,
  };
}
...
export async function getOverviewForUser(userId, { agentId, range } = {}) {
  ...
  const { assistantTimes, exact } = await loadBundle(userId, {
    agentId,
    since,
  });
...
export async function getTopicsForUser(userId, { agentId } = {}) {
  ...
  const since = sinceForRange(rangeInfo);
  const { exact } = await loadBundle(userId, {
    agentId,
    since,
  });
  const topics = buildTopicsFromExact(exact, []);
...
export async function getTrendsForUser(
  ...
  const agents = await resolveAgents(userId, agentId);
  const agentIds = agents.map((agent) => agent.id);
  const rangeSpec = buildRangeSpec({
    start: trendStart,
    bucket,
    buckets,
  });
  const trends = await loadTrendPointsSql(agentIds, start, rangeSpec);

  return {
    period: safePeriod,
    points: trends.points.map(({ date, conversations: count, messages }) => ({
      date,
      conversations: count,
      messages,
    })),
...
export async function getDashboardForUser(userId, { agentId, range } = {}) {
  return withAnalyticsTimeout(
    (async () => {
      ...
      const { dashboard } = await buildDashboardAnalytics({
        ...bundle,
        rangeInfo,
        scoped,
        since,
...
export async function getDashboardForPlatform({ range } = {}) {
  return withAnalyticsTimeout(
    (async () => {
      ...
      const allAgentIds = (
        await prisma.agent.findMany({ select: { id: true } })
      ).map((agent) => agent.id);

      const activeIds = since
        ? (
      ...
      const [exact, assistantTimes] = await Promise.all([
        loadExactKpis(allAgentIds, since),
        loadAssistantTimes(allAgentIds, since),
      ]);
      ...
      const { dashboard, rangeSpec, chatTrendPoints } = await buildDashboardAnalytics({
        agents,
        assistantTimes,
        exact,
        rangeInfo,
      ...
      const [totals, growthParts] = await Promise.all([
        ...
        loadPlatformGrowthSql(rangeSpec),
      ]);

      const [users, workspaces, agentCount, liveEmbeds, conversationsTotal] =
        totals;

      const chatPoints = chatTrendPoints.map((point) => ({
        date: point.date,
        label: point.label,
        count: point.conversations,
      }));
