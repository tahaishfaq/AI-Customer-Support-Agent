...
import { detectCapabilityAsk } from "@/lib/services/ai/intent-clarify";
...
function detectSourceSignals(t) {
  ...
  // Capability / confirmation ("do you have GitHub access?") is not inventory.
  const capabilityAsk = detectCapabilityAsk(t);

  // GitHub / connected-MCP inventory asks — not store knowledge, not Confirm (READ).
  // Include common typos (gtihub) and spaced "git hub".
  // Capability-only asks must not set wantsGithub (that would offer live MCP).
  const mentionsGithub =
    /\bgit[\s-]*hubs?\b/.test(t) ||
    /\bgtihub\b/.test(t) ||
    /\bgithb\b/.test(t);
  const inventoryGithub =
  ...
  const wantsGithub = !capabilityAsk && (mentionsGithub || inventoryGithub);
  ...
  // "GitHub profile" is not an Aide ACCOUNT tool ask — keep MCP tools available.
  if (
    /\b(preference|preferences|newsletter|settings)\b/.test(t) ||
    (/\bprofile\b/.test(t) && !wantsGithub) ||
    /\b(update|change|set|save)\s+(my\s+)?(preference|preferences|newsletter|profile|settings|account)\b/.test(
      t
    )
  ) {
 ...
 *   signals: { wantsWeb: boolean, storeFact: boolean, wantsCompare: boolean, generalAsk: boolean, wantsGithub: boolean, capabilityAsk: boolean },
 *   entities: string[],
 * }}
 */
...
/**
 * Server-authored capability answer context — no live probe.
 * @param {{ toolNames?: string[], connected?: boolean, platform?: string }} opts
 */
export function capabilityAskSystemAddon(opts = {}) {
  const platform = String(opts.platform || "GitHub").trim() || "GitHub";
  const names = (Array.isArray(opts.toolNames) ? opts.toolNames : [])
    .map((n) => String(n || "").trim())
    .filter(Boolean)
    .slice(0, 12);
  const connected = Boolean(opts.connected) && names.length > 0;
  if (connected) {
    return [
      `## Capability check (server): ${platform}`,
      `The user asked whether you have access/connection — not for live ${platform} data.`,
      `${platform} tools are configured for this agent: ${names.join(", ")}.`,
      "Answer briefly that yes, those tools are configured/connected for this agent.",
      "Do NOT call any tools this turn. Do not claim you just verified a live token.",
      "Say you can use them when the user asks for live profile, repos, or similar data.",
    ].join(" ");
  }
  return [
    `## Capability check (server): ${platform}`,
    `The user asked whether you have access/connection — not for live ${platform} data.`,
    `No ${platform} MCP tools are enabled for this agent right now.`,
    "Answer briefly that you do not currently have those tools connected.",
    "Do NOT call any tools this turn. Do not invent live profile or repository data.",
    "Suggest the site owner connect GitHub under Tools if they need live access.",
  ].join(" ");
}

/** True when an action is a GitHub MCP tool. */
export function isGithubMcpAction(action) {
  if (!action?._mcp) return false;
  return /github/i.test(String(action?.name || ""));
}
  ...
  return list.filter((c) => {
    ...
    if (stripWrites) {
      ...
      if (risk === "WRITE" || risk === "DESTRUCTIVE") {
        // GitHub MCP writes are live connected tools with Confirm/PEP — not conceptual GENERAL prose.
        // Keep them when the turn is a GitHub ask; still require confirmation at invoke.
        const keepGithubWrite =
          Boolean(decision?.signals?.wantsGithub) &&
          /github/i.test(String(c?.name || ""));
        if (keepGithubWrite) {
          return true;
        }
        return false;
...
export function sourceRouteSystemAddon(decision) {
  ...
  const githubAddon = decision?.signals?.wantsGithub
    ? [
        ...
        "For repository lists (user asked for repos / repositories / html_url): you MUST call search_repositories (e.g. user:USERNAME) or list_repositories — not list_releases, not list_repository_collaborators, not get_file_contents alone.",
        "get_me is only for profile/who-am-I; after get_me, if the user wants repos, call search_repositories next in the same turn when possible.",
        "When the user asks to create/update a GitHub repo (or names create_repository): call the matching WRITE GitHub MCP tool with the exact name/visibility from their message. Do not give manual github.com steps and do not call request_handoff when that write tool is listed.",
        "If name/visibility are already in the thread from the user, use them — do not re-ask. Confirm is required for WRITE; wait for Confirm, do not hand off.",
        "Name conflict flow (strict): (1) First create attempt must use the exact name the user asked for — never invent suffixes like _2 or -v2 yourself. (2) If create_repository fails because that name already exists: do NOT call create_repository again in the same turn. Reply in text only: say that name is already taken, offer 1–2 alternate name options, and invite the user to reply with an option or type their own new name. (3) Only after the user picks or types a new name in a later message may you call create_repository with that chosen name (then Confirm). Never auto-rename and jump straight to Confirm.",
        "Only say created when the tool result clearly shows a new repo was created.",
        "Show at most 5 repos this turn with name and html_url/live URL when the tool returns them; say the list is partial and offer another batch.",
        "Do not call list_branches for every repo in one turn — only for a repo the user named.",
