...
export function sourceRouteSystemAddon(decision) {
  ...
  const githubAddon = decision?.signals?.wantsGithub
    ? [
        ...
        "When the user asks to create/update a GitHub repo (or names create_repository): call the matching WRITE GitHub MCP tool with name/visibility from their message. Do not give manual github.com steps and do not call request_handoff when that write tool is listed.",
        "If name/visibility are already in the thread, use them — do not re-ask. Confirm is required for WRITE; wait for Confirm, do not hand off.",
        "If create_repository fails because that name already exists (or a name conflict / name taken error): do not claim the repo was created. Tell the user that name is already taken and suggest 1–2 alternate names they can confirm (e.g. Harness_Agent_2 or Harness-Agent-v2). Only say created when the tool result clearly shows a new repo was created.",
