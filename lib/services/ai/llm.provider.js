...
export const HOSTED_WEB_SEARCH_TIMEOUT_MS = 12_000;

function createWebSearchSignal(parentSignal) {
  const timeoutSignal = AbortSignal.timeout(HOSTED_WEB_SEARCH_TIMEOUT_MS);
  return {
    signal: parentSignal
...
export async function responsesTurn({ system, messages, signal }) {
  ...
  const requestSignal = createWebSearchSignal(signal);
  try {
    ...
    if (requestSignal.timeoutSignal.aborted && !signal?.aborted) {
...
/**
 * Stream one chat turn with optional tools. Content deltas fire via onDelta
 * only while no tool_calls are accumulating (so tool rounds stay quiet).
 * @param {{
 *   system: string,
 *   messages: Array<Record<string, unknown>>,
 *   tools?: Array<object>,
 ...
 * @returns {Promise<{ content: string|null, toolCalls: Array|null, latencyMs: number }>}
 */
export async function chatCompletionStreamTurn({
  system,
  messages,
  tools,
  signal,
  ...
  try {
    ...
    if (isAbortError(error) || signal?.aborted) {
      safeLogError("OpenAI chatCompletionStreamTurn aborted", {
        code: "ABORTED",
        durationMs,
      });
      throw httpError(499, "Request cancelled", { code: "ABORTED" });
    ...
    if (isTimeoutError(error)) {
      safeLogError("OpenAI chatCompletionStreamTurn timeout", {
        code: "TIMEOUT",
        durationMs,
      });
      throw httpError(
    ...
    safeLogError("OpenAI chatCompletionStreamTurn failed", {
      code: error?.code || error?.status || undefined,
      durationMs,
    });
    throw httpError(502, "AI provider request failed");
...
export async function* chatCompletionStream({ system, messages, signal }) {
  ...
  try {
    ...
    safeLogError("OpenAI chatCompletionStream failed", {
      code: error?.code || error?.status || undefined,
      durationMs,
      providerMessage: String(error?.message || "").slice(0, 180),
    });
    throw httpError(502, "AI provider request failed");
  }
}
