 ...
 *   pendingUserMessage?: { clientMessageId: string, content: string }|null,
 * }} opts
 * `pendingUserMessage`: this turn's USER message while it is still being saved. It is left out of
 * the history query and added in memory, so the history equals a read made after the save.
 */
export async function buildAgentTurnContext({
  ...
  /** ms per step (timing only; surfaced to studio latency audits). */
  const timings = {};
  ...
  const [knowledgeDocs, recentMessages, deskNotes, actionRuntime, latestCrawlJob] =
    await Promise.all([
      timed("knowledgeDocs", prisma.knowledgeDocument.findMany({
        where: { agentId },
      ...
      prisma.message
        .findMany({
          where: {
            conversationId,
            role: { not: "INTERNAL" },
            ...(pendingUserMessage?.clientMessageId
              ? {
                  // Explicit null branch: `not` alone would also drop rows without a clientMessageId.
                  OR: [
                    { clientMessageId: null },
                    { clientMessageId: { not: pendingUserMessage.clientMessageId } },
                  ],
                }
              : {}),
          },
          orderBy: { createdAt: "desc" },
          take: pendingUserMessage ? MAX_HISTORY_MESSAGES - 1 : MAX_HISTORY_MESSAGES,
          select: { id: true, role: true, content: true },
        })
        .then((rows) =>
          pendingUserMessage
            ? [{ id: null, role: "USER", content: pendingUserMessage.content }, ...rows]
            : rows
        ),
      ...
      timed("actions", listEnabledActionsForAgent(agentId, { agent })),
  ...
  if (sourceResolved?.preference === "web") {
    ...
    // Follow-up inventory / write-param without "github" in the text — reuse sticky source.
    ...
    const writeFollowUp = detectGithubWriteFollowUp(effectiveMessage);
    if (sticky === "github" && (inventoryAsk || writeFollowUp)) {
      sourceDecision = {
        ...sourceDecision,
        route: "GENERAL",
        mayInvokeWebSearch: false,
  ...
  if (!isTrivialGreeting && knowledgeDocs.length) {
    ...
    timings.knowledgeSelectMs = Date.now() - selectStarted;
  ...
  // Knowledge-first must not hide a tool the question is plainly about (e.g. "campaign CAMP-200
  // status" while some website chunk matched): re-offer READ HTTP tools that match the question.
  if (sourceResolved?.preference !== "knowledge") {
    const matching = new Set(
      relevantToolNames(enabledActions.filter((a) => !a?._mcp), contentForLlm(routeUtterance))
    );
    const missing = enabledActions.filter(
      (a) => matching.has(a.name) && !offeredActions.some((o) => o.name === a.name)
    );
    if (missing.length) offeredActions = [...offeredActions, ...missing];
  }
  ...
  // #region agent log
  {
    const { inventoryAsk, explicitGithub } = detectSourceAskSignals(
      contentForLlm(effectiveMessage)
    );
    const stickyDbg = inferStickySourcePreference(recentMessages, {
      currentUtterance: contentForLlm(effectiveMessage),
    });
    const writeOffered = offeredActions.filter((a) =>
      ["WRITE", "DESTRUCTIVE"].includes(String(a?.riskLevel || "").toUpperCase())
    );
    const githubOffered = offeredActions.filter(isGithubMcpAction).map((a) => a.name);
    const createOffered = githubOffered.some((n) => /create_repositor/i.test(n));
    fetch("http://127.0.0.1:7921/ingest/84e093c9-c14f-4ada-99a7-450980e35e0c", {
      ...
      body: JSON.stringify({
        ...
        runId: "pre-fix",
        hypothesisId: "H1-H3",
        location: "turn-context.js:offeredActions",
        message: "turn tool offer after filters",
        data: {
          utterance: String(contentForLlm(effectiveMessage) || "").slice(0, 160),
          route: sourceDecision.route,
          wantsGithub: Boolean(sourceDecision.signals?.wantsGithub),
          inventoryAsk,
          explicitGithub,
          sticky: stickyDbg,
          usedKnowledgeCount: usedKnowledge.length,
          offeredCount: offeredActions.length,
          githubOffered: githubOffered.slice(0, 20),
          createRepoOffered: createOffered,
          writeOfferedCount: writeOffered.length,
          writeOfferedNames: writeOffered.map((a) => a.name).slice(0, 10),
          hasHandoff: offeredActions.some(
            (a) =>
