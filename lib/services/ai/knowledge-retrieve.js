...
import { knowledgeUntrustedBanner } from "../../actions/untrusted-result.js";
...
/** Max chunks from a single doc in one turn (stops one crawl eating the budget). */
export const MAX_CHUNKS_PER_DOC = 4;
/** No single doc may use more than this share of the char budget. */
export const MAX_DOC_BUDGET_SHARE = 0.45;
...
/** Optional env: KNOWLEDGE_MAX_CHUNKS_PER_DOC (1–20). Default 4. */
export function resolveMaxChunksPerDoc() {
  const fromEnv = Number.parseInt(process.env.KNOWLEDGE_MAX_CHUNKS_PER_DOC || "", 10);
  if (Number.isFinite(fromEnv) && fromEnv >= 1) {
...
function chunkHeading(chunk) {
  const name = chunk.name || "Knowledge";
  const type = chunk.type || "TEXT";
  const base = `### ${name} (${type})`;
  if (String(type).toUpperCase() === "WEB") {
    const src = String(chunk.sourceUrl || chunk.origin || "").trim();
...
/**
 * Round-robin pack so multiple docs share the char budget (W3-1 / P1).
 * @param {{ chunk: object, score: number }[]} rankedRows
 */
function packChunksFair(rankedRows, budget, packLimit) {
  const headerLen = `## Agent knowledge\n${knowledgeUntrustedBanner()}\n\n`.length;
  const maxPerDoc = resolveMaxChunksPerDoc();
  const maxDocChars = Math.max(
    400,
    Math.floor(budget * MAX_DOC_BUDGET_SHARE)
  );

  /** @type {Map<string, typeof rankedRows[0]["chunk"][]>} */
  const byDoc = new Map();
  for (const row of rankedRows) {
    const id = row.chunk.docId;
    if (!byDoc.has(id)) byDoc.set(id, []);
    byDoc.get(id).push(row.chunk);
  }

  const docOrder = [...byDoc.keys()].sort((a, b) => {
    const scoreA = rankedRows.find((r) => r.chunk.docId === a)?.score ?? 0;
    const scoreB = rankedRows.find((r) => r.chunk.docId === b)?.score ?? 0;
    return scoreB - scoreA;
  });

  const queues = docOrder.map((id) => [...byDoc.get(id)]);
  const perDocCount = new Map();
  const perDocChars = new Map();
  const packed = [];
  let usedChars = headerLen;

  while (packed.length < packLimit) {
    let added = false;
    for (let i = 0; i < docOrder.length; i += 1) {
      const docId = docOrder[i];
      const queue = queues[i];
      while (queue.length) {
        if ((perDocCount.get(docId) || 0) >= maxPerDoc) break;
        if ((perDocChars.get(docId) || 0) >= maxDocChars) break;

        const chunk = queue.shift();
        if (isNearDuplicate(chunk, packed)) continue;

        const piece = chunkPiece(chunk);
        if (usedChars + piece.length > budget) {
          if (packed.length === 0) {
            ...
            if (room > 40) {
              ...
              usedChars += room + 80;
              perDocCount.set(docId, (perDocCount.get(docId) || 0) + 1);
              perDocChars.set(docId, (perDocChars.get(docId) || 0) + room);
              added = true;
            }
          }
          break;
        ...
        perDocCount.set(docId, (perDocCount.get(docId) || 0) + 1);
        perDocChars.set(docId, (perDocChars.get(docId) || 0) + piece.length);
        added = true;
        break;
      }
    }
...
export function formatClarifyQuestion(clarify) {
  ...
  const numbered = list
    .map((c, i) => `${i + 1}. **${c.phrase}**`)
    .join("\n");
  return `Your message looks unclear (possible spelling). Did you mean one of these?\n\n${numbered}\n\nReply with a number (1, 2, …) or the topic name, or rephrase your question.`;
}
...
/**
 * If user said "yes" or picked a numbered clarify option, rewrite the retrieve query.
  ...
  for (const msg of recentMessagesDesc) {
    ...
    if (numMatch) {
      ...
      if (idx >= 0 && idx < phrases.length) {
        return `Tell me about ${phrases[idx]}`;
      }
    }

    for (const phrase of phrases) {
      if (t === phrase.toLowerCase() || t.includes(phrase.toLowerCase())) {
        return `Tell me about ${phrase}`;
      }
    }

    if (phrases.length === 1 && isAffirmativeReply(raw)) {
 ...
 * when allowSoftFallback is true (STORE-style turns only from callers).
 * (True semantic embeddings remain F10.)
 *
 * @param {{ docs: object[], query: string, maxChars?: number, siteKnowledgeOrigin?: string|null, topicHint?: string|null, recentMessages?: { role: string, content: string }[], allowSoftFallback?: boolean }} args
 */
export function selectKnowledgeChunks({
  ...
  let rankedRows;
  if (softFallback) {
    // GitHub / GENERAL callers pass allowSoftFallback:false so Brandly (etc.)
    // newest docs are not stuffed or shown as "Used knowledge".
  ...
  const packed = packChunksFair(rankedRows, budget, packLimit);
