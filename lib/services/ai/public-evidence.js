...
/** A price next to a plan word ("Pro $79", "$29/month"). A stray amount elsewhere is not plan evidence. */
const PLAN_PRICE_NEAR =
  /\b(?:plans?|pricing|prices?|tiers?|packages?|subscriptions?|starter|basic|pro|premium|business|enterprise|free)\b[^\n.!?]{0,30}(?:[$ۣ]\s?\d|\b\d[\d,.]*\s?(?:usd|eur|gbp)\b)|[$ۣ]\s?\d[\d,.]*[^\n]{0,24}(?:\/\s?(?:mo|month|yr|year|user)|\bper (?:month|year|user)\b|\bmonthly\b|\byearly\b|\bannual)/i;
/** Crawl summaries sometimes state pricing is missing: that is not evidence. */
const PRICING_UNSPECIFIED = /\b(?:pricing|prices?|plans?)\b[^\n]{0,24}\b(?:not specified|not available|not listed|unknown|n\/a)\b/i;
...
export function decidePublicEvidence({
  ...
  const evidence = (Array.isArray(knowledgeDocs) ? knowledgeDocs : []).filter(
    (doc) => {
      ...
      return (
        PUBLIC_PLAN_TERMS.test(content) &&
        PUBLIC_PRICE_EVIDENCE.test(content) &&
        PLAN_PRICE_NEAR.test(content) &&
        !PRICING_UNSPECIFIED.test(content)
      );
    }
  );
