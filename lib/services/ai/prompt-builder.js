...
export const RESPONSE_RULES_GROUNDING = [
  ...
  "When knowledge document titles are relevant, you may mention them briefly (e.g. according to Refund FAQ).",
 ...
 * @param {{ replyLanguage?: string, answerStyle?: AnswerStyle|string }} [opts]
 */
...
/**
 * Full chat system prompt: role overlay → Response rules → optional F08 knowledge.
 * @param {{
...
export function buildChatSystemPrompt({
  ...
  const rules = buildResponseRules({ replyLanguage, answerStyle: style });
  const base = `${overlay}\n\n${RESPONSE_RULES_SECTION}\n${rules}`;
  const knowledge = String(knowledgeText || "").trim();
  const full = knowledge ? `${base}\n\n${knowledge}` : base;
