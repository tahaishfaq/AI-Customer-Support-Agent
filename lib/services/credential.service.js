...
export function serializeCredentialForOwner(cred) {
  ...
  return {
    ...
    // Never return ciphertext, plaintext, or raw secret.
  };
}
...
export async function revokeCredentialForAgent(agentId, credentialId, userId) {
  ...
  // Connections keep the row but drop the secret reference.
  await prisma.integrationConnectionRevision.updateMany({
    where: { credentialId },
    data: { credentialId: null },
  });
