...
function assertPublishSafeOrThrow(actionLike) {
  const check = assertAccountToolPublishSafe(actionLike);
  if (!check.ok) {
    throw httpError(400, check.message, { code: check.code });
  }
}

async function requireManagedAgent(agentId, userId) {
  const agent = await getAgentForUser(agentId, userId, { mutate: true });
  if (!canManageAgentActions({ userId, agent, canManage: true })) {
...
function actionCreateData(agentId, data) {
  ...
  return {
    ...
    requestContentType: data.requestContentType ?? "application/json",
    requestBodyTemplate: data.requestBodyTemplate ?? undefined,
    parameterBindings: data.parameterBindings ?? undefined,
    inputSchemaJson: data.inputSchemaJson ?? undefined,
    outputSchemaJson: data.outputSchemaJson ?? undefined,
    responseProjectionJson: data.responseProjectionJson ?? undefined,
...
export async function createActionForAgent(agentId, userId, data) {
  ...
  const projectionCheck = validateResponseProjection(data.responseProjectionJson);
  if (!projectionCheck.ok) {
    throw httpError(400, projectionCheck.error, { code: "OUTPUT_PROJECTION_INVALID" });
  }
  ...
  if (data.connectionId) {
    const connection = await resolveActionConnection(
      { connectionId: data.connectionId, urlTemplate: data.urlTemplate },
      agent.workspaceId
    );
    if (!connection || !isActionDestinationAllowed(data.urlTemplate, connection.revision)) {
      throw httpError(400, "Action destination is not allowed by the connection");
    }
  }
  const createPayload = actionCreateData(agentId, data);
  assertPublishSafeOrThrow(createPayload);
  try {
...
export async function updateActionForAgent(agentId, actionId, userId, data) {
  ...
  if (existing.publishedRevisionId) {
    throw httpError(
      409,
      "Published action is immutable; create a draft revision first.",
      { code: "ACTION_REVISION_IMMUTABLE" }
    );
  }

  if (data.responseProjectionJson !== undefined) {
    const projectionCheck = validateResponseProjection(data.responseProjectionJson);
    if (!projectionCheck.ok) {
      throw httpError(400, projectionCheck.error, { code: "OUTPUT_PROJECTION_INVALID" });
    }
  ...
  const connectionId =
    data.connectionId !== undefined ? data.connectionId : existing.connectionId;
  if (connectionId) {
    await resolveActionConnection(
      {
        connectionId,
        urlTemplate: data.urlTemplate !== undefined ? data.urlTemplate : existing.urlTemplate,
      },
      agent.workspaceId
    );
  }
  ...
  const bumpVersion =
    ...
    data.requestContentType !== undefined ||
    data.requestBodyTemplate !== undefined ||
    data.parameterBindings !== undefined ||
    data.responseProjectionJson !== undefined ||
    ...
    const action = await prisma.agentAction.update({
      ...
      data: {
        ...
        ...(data.outputSchemaJson !== undefined
          ...
          : {}),
        ...(data.responseProjectionJson !== undefined
          ? { responseProjectionJson: data.responseProjectionJson }
          : {}),
        ...(data.enabled !== undefined ? { enabled: data.enabled } : {}),
...
export async function testActionForAgent(agentId, actionId, userId, { args = {} } = {}) {
  ...
  const actionRow = await prisma.agentAction.findFirst({
    where: { id: actionId, agentId },
  });
  if (!actionRow) throw httpError(404, "Action not found");
  const action = await materializePublishedAction(actionRow);
  if (!action.enabled) {
    throw httpError(400, "Action is disabled");
  }

  const limited = await rateLimit(`actions:outbound:${agentId}`, actionOutboundLimitOpts());
  if (!limited.ok) {
  ...
  if (isGetMethod(action.method)) {
    const cached = await getCachedGetResult(action.id, args);
    if (cached) {
      await prisma.toolRun.create({
        data: {
          ...
          actionRevisionId: action.actionRevisionId || null,
  ...
  let credential = null;
  const connection = await resolveActionConnection(action, agent.workspaceId);
  const runtimeCredentialId = connection?.credentialId || action.credentialId;
  if (runtimeCredentialId && agent.workspaceId) {
    try {
  ...
  const result = await withOutboundSlot(agentId, () =>
    executeHttpAction({
      ...
      responseProjectionJson: action.responseProjectionJson,
  ...
  if (isGetMethod(action.method) && result?.ok) {
    await setCachedGetResult(action.id, args, result);
  }
...
export async function listToolRunsForAgent(agentId, userId, { take = 30 } = {}) {
  ...
  return runs.map((run) => ({
    ...
    actionRevisionId: run.actionRevisionId,
    status: run.status,
