...
export async function sendChatMessage(agentId, userIdOrOptions, maybePayload) {
  ...
  const canTokenStream =
    wantStream && !publicAccess && !enabledActions.length;

  if (canTokenStream && streamEmit) {
    streamEmit({
      type: "meta",
      data: {
        conversationId: conversation.id,
        ...
        try {
          if (canTokenStream) {
            ...
            for await (const delta of chatCompletionStream({
              ...
              content += delta;
              streamEmit({ type: "delta", data: { text: delta } });
            }
            ...
            reply = await chatCompletionWithTools({
              system,
              messages: llmMessages,
  ...
  let ackMessage = null;
  if (publicAccess && !degraded && needHuman) {
    const handoff = await tryTriggerHandoff({
      conversationId: conversation.id,
      publicAgentId: agentId,
