...
export async function sendChatMessage(agentId, userIdOrOptions, maybePayload) {
  ...
  const streamKnowledgeActivity = wantStream && !publicAccess && streamEmit && knowledgeDocs.length;
  if (streamKnowledgeActivity) {
    streamEmit({
      type: "tool",
      data: {
        kind: "agent_activity",
  ...
  try {
    ...
    system = applySourceRouteToSystem(system, sourceDecision);
