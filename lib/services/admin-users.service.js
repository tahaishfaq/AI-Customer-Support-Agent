...
import { revokeAllRealtimeSessions } from "@/lib/realtime/session.service";
...
export async function getAdminUser(id) {
  ...
  const conversations =
    user.role === "ADMIN"
      ? null
      : await getAdminConversationQuota(id, user.role);
  const billing =
    user.role === "ADMIN"
      ? null
      : await getBillingSnapshot(id, user.role);
...
export async function setUserStatus(id, status, { actorId } = {}) {
  ...
  await invalidatePublicUserCache(id);
  if (status !== "ACTIVE") {
    await revokeAllRealtimeSessions(id);
  }
  if (status === "ACTIVE") {
