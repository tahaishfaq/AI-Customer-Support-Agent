...
export async function createTurnRun({ agentId, conversationId, workspaceId = null, clientMessageId = null, requestId = null, ownershipVersion = 0, status = "ACCEPTED" }) {
  if (!agentId || !conversationId) return null;
  try {
    return await prisma.turnRun.create({
      data: { agentId, conversationId, workspaceId, clientMessageId, requestId, ownershipVersion, status },
    });
  } catch (error) {
