...
/** Once per process — only if SELECT proves the column is missing. */
...
async function ensureReservedAdminEmailColumn() {
  ...
  reservedColumnReady = (async () => {
    try {
      await prisma.$queryRaw`
        SELECT "reservedAdminEmail"
      ...
      if (!/reservedAdminEmail|does not exist|42703/i.test(message)) {
        throw error;
      }
    }
...
async function ensureRow() {
  // Prefer plain upsert — do not ALTER on every cold request (locks Neon).
  try {
...
export async function getPlatformSettings() {
  ...
  if (
    settingsCache &&
    now - settingsCacheAt < SETTINGS_CACHE_MS
  ) {
    return settingsCache;
  }

  try {
    // Hot path: SELECT only. Avoid INSERT/ALTER on every cold miss.
    let row = await readRow();
    if (!row) {
      await ensureRow();
      row = await readRow();
    }
    settingsCache = toSettings(row);
    settingsCacheAt = now;
    return settingsCache;
  } catch (error) {
    const message = String(error?.message || error);
