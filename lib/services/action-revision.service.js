...
import { assertAccountToolPublishSafe } from "@/lib/actions/access-class";
...
async function publishRevision(agentId, actionId, revisionId, userId, { allowRetired = false } = {}) {
  ...
  const publishSafe = assertAccountToolPublishSafe(revision);
  if (!publishSafe.ok) {
    throw httpError(400, publishSafe.message, { code: publishSafe.code });
  }
...
/** `publishedRevisions`: optional preloaded PUBLISHED rows; matched with the same id/action/state rules. */
export async function materializePublishedAction(action, { publishedRevisions = null } = {}) {
  if (!action?.publishedRevisionId) return action;
  const revision = Array.isArray(publishedRevisions)
    ? publishedRevisions.find(
        (row) =>
          row.id === action.publishedRevisionId &&
          row.actionId === action.id &&
          row.state === "PUBLISHED"
      ) || null
    : await prisma.actionRevision.findFirst({
