...
/** Shown on the Knowledge page when a JavaScript-only site needs a browser this server lacks. */
export const BROWSER_UNAVAILABLE_MESSAGE =
  "This website builds its pages with JavaScript, and this server has no browser to render them, so no text could be read. Add your key pages as text or upload documents, or ask your admin to deploy with the Docker image (includes Chromium).";
...
export async function crawlPublicOriginBrowser(origin, options = {}) {
  ...
  try {
    ...
    throw crawlHttpError(503, BROWSER_UNAVAILABLE_MESSAGE, {
  ...
  try {
    ...
    // Launch detail is for server logs only; owners get the actionable message.
    console.warn("[crawl] Chromium launch failed:", String(error?.message || "launch_failed").slice(0, 200));
    throw crawlHttpError(503, BROWSER_UNAVAILABLE_MESSAGE, {
      code: "CRAWL_BROWSER_UNAVAILABLE",
    });
  }
