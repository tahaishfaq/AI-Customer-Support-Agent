...
/** `lastError` prefix for credential failures; cleared by reconnect, credential change or a good probe. */
export const MCP_AUTH_ERROR_PREFIX = "MCP_AUTH:";
...
/** Risk the gateway actually enforces: stored value, tightened for evident side-effect names. */
function effectiveMcpToolRisk(tool) {
  const riskLevel = stricterMcpRisk(tool.riskLevel, detectMcpWriteRisk(tool.name) || "READ");
  return { riskLevel, requiresConfirmation: Boolean(tool.requiresConfirmation) || riskLevel !== "READ" };
}

export function serializeMcpTool(tool) {
  if (!tool) return null;
  const effective = effectiveMcpToolRisk(tool);
  return {
    id: tool.id,
    ...
    const server = await prisma.agentMcpServer.update({
      ...
      data: {
        ...
        ...(data.url !== undefined
          ...
          : {}),
        // A new credential gets a fresh chance: clear a recorded auth failure.
        ...(data.credentialId !== undefined ? { lastError: null } : {}),
...
/**
 * Ephemeral tools/list — no AgentMcpServer row. Prefer before Save.
 * Auth uses credentialId only (never accepts plaintext secrets in the body).
 */
export async function probeDraftMcpServerForAgent(agentId, userId, data) {
  const agent = await requireManagedAgent(agentId, userId);
  const url = String(data.url || "").trim();
  const authType = data.authType || "NONE";
  const headerName = data.headerName || null;
  const credentialId = data.credentialId || null;

  if (authType !== "NONE") {
    if (!credentialId) {
      throw httpError(400, "credentialId required for authenticated MCP probe", {
        code: "CREDENTIAL_REQUIRED",
      });
    }
    await assertCredentialInWorkspace(credentialId, agent.workspaceId);
  }

  let secret = null;
  if (credentialId && authType !== "NONE") {
    const cred = await loadDecryptedCredential(credentialId, agent.workspaceId);
    secret = cred?.plaintext || null;
    if (!secret) {
      throw httpError(400, "Credential is missing or empty", {
  ...
  try {
    ...
    logMcpAudit("mcp.server_probe_draft", {
      agentId,
      status: "ok",
      code: String(probed.tools?.length ?? 0),
    });
    return {
      ok: true,
      frozenHost: probed.frozenHost,
      serverInfo: probed.serverInfo,
      protocolVersion: probed.protocolVersion,
      tools: (probed.tools || []).map((t) => {
        const risk = inferMcpToolRisk(t.name, t.annotations);
        return {
          name: t.name,
      ...
      discovered: probed.tools?.length ?? 0,
    };
  } catch (err) {
    logMcpAudit("mcp.server_probe_draft", {
      agentId,
      status: "error",
      code: err?.details?.code || err?.code || "MCP_PROBE_FAILED",
    });
    const status = err?.status && err.status >= 400 && err.status < 600 ? err.status : 400;
    throw httpError(status, err?.message || "MCP draft probe failed", {
      code: err?.details?.code || err?.code || "MCP_PROBE_FAILED",
    });
  }
}
...
export async function probeMcpServerForAgent(agentId, serverId, userId) {
  ...
  try {
    ...
    for (const discovered of probed.tools) {
      ...
      const risk = inferMcpToolRisk(discovered.name, discovered.annotations);
      ...
      if (prev) {
        await prisma.agentMcpTool.update({
          ...
          data: {
            ...
            // Keep enabled; reclassify auto-seeded risk, only ever tighten an owner override.
            ...reclassifyStoredMcpToolRisk(prev, discovered.annotations),
          },
        });
      } else {
    ...
    await prisma.agentMcpServer.update({
      ...
      data: {
        lastProbeAt: new Date(),
        lastError: (err.code === "MCP_AUTH" ? `${MCP_AUTH_ERROR_PREFIX} ${message}` : message).slice(0, 500),
      },
    });
    logMcpAudit("mcp.probe", {
      agentId,
 ...
 */
export async function listEnabledMcpToolsForAgent(agentId, { agent: knownAgent = null } = {}) {
  if (!agentId) return [];
  const agent =
    knownAgent?.id === agentId
      ...
      : await prisma.agent.findUnique({
          where: { id: agentId },
          select: { id: true, actionsEnabled: true, workspaceId: true },
        });
...
export function mcpToolToActionShape(tool, workspaceId = null) {
  ...
  // Runtime guard: a stored row that under-rates an obvious side-effect name is tightened here,
  // so policy never treats e.g. merge_pull_request as an owner READ.
  const { riskLevel, requiresConfirmation } = effectiveMcpToolRisk(tool);
  return {
    id: tool.id,
    agentId: server.agentId,
    ...
    _mcp: {
      ...
      credentialId: server.credentialId,
      transport: server.transport,
      authFailed: String(server.lastError || "").startsWith(MCP_AUTH_ERROR_PREFIX),
    },
  };
}
...
export async function executeMcpToolAction({
  ...
  onDispatch,
  dispatchSignal,
  /** { fullList?: boolean, onProgress?: (p) => void } — compact list results; page READ lists. */
  listOptions = null,
}) {
  const meta = action?._mcp;
  if (!meta) throw httpError(400, "Not an MCP tool");
  // Chunked fetching only for READ list tools that expose page/size args, when "all" was asked.
  const pagination =
    listOptions?.fullList && String(action.riskLevel || "").toUpperCase() === "READ"
      ? paginationParams(action.inputSchemaJson)
      : null;
  const firstArgs = pagination
    ? { ...args, [pagination.pageKey]: 1, [pagination.sizeKey]: LIST_PAGE_SIZE }
    : args;

  const serverLimited = await rateLimit(
  ...
  try {
    ...
    let bodyText = formatMcpToolResultForModel(result);
    const isError = Boolean(result?.isError);
    let list = null;
    const parsed = !isError && listOptions ? parseListPayload(mcpResultText(result)) : null;
    if (parsed) {
      const { items, fetchedAll } = await collectPagedList({
        first: parsed,
        paginate: Boolean(pagination),
        startedAt: started,
        onProgress: listOptions.onProgress,
        fetchPage: async (page) => {
          if (dispatchSignal?.aborted) return null;
          const pageLimited = await rateLimit(`mcp:outbound:${meta.serverId}`, mcpServerOutboundLimitOpts());
          if (!pageLimited.ok) return null;
          const next = await callMcpTool({
            url: meta.url,
            authType: meta.authType,
            headerName: meta.headerName,
            secret,
            ...
            toolName: meta.remoteName,
            dispatchSignal,
            args: { ...args, [pagination.pageKey]: page, [pagination.sizeKey]: LIST_PAGE_SIZE },
            signal,
          });
          return next?.isError ? null : mcpResultText(next);
        },
      });
      bodyText = formatCompactListForModel(items, { total: parsed.total, fetchedAll });
      list = { items, total: parsed.total, fetchedAll };
    }
    ...
    return {
      ok: !isError,
      status: isError ? "ERROR" : "OK",
      // Protocol errors are not HTTP success — null avoids stripping MCP detail
      // in formatToolResultForModel (which only includes detail for 4xx).
      httpStatus: isError ? null : 200,
      durationMs: Date.now() - started,
      bodyText,
    ...
    // Expired/invalid credential: mark the server so its tools stop being offered until reconnect.
    if (err.code === "MCP_AUTH") {
      await prisma.agentMcpServer
        .update({
          where: { id: meta.serverId },
          data: { lastError: `${MCP_AUTH_ERROR_PREFIX} ${err.message || "authentication failed"}`.slice(0, 500) },
        })
        .catch(() => null);
    }
    logMcpAudit("mcp.tool_call", {
